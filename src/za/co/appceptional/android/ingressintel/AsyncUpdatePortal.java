 ...
 * @author Yaniv Inbar
 */
class AsyncUpdatePortal extends PortalAsyncTask
{
	PortalInfo portalInfo;
	
	AsyncUpdatePortal(PortalsFragment fragment, PortalInfo portalInfo)
	{
		super(fragment);
	...
	protected void doInBackground() throws IOException {
		...
		String sqlstring =
				"UPDATE " + tableName + 
				" SET Name='" + portalInfo.name + "'," +
				"Location='" + portalInfo.location + "'," +
				"DateModified='" + portalInfo.dateModified + "'," +
				"Team='" + portalInfo.team + "'," +
				"Level=" + portalInfo.level + " WHERE " +
				"ROWID = '" + portalInfo.id + "'";
		
		Sql sql = client.query().sql(sqlstring);
		Sqlresponse r = sql.execute ();
		
		sqlstring = "select ROWID, Name, Location, DateCreated, DateModified, Team, Level from " + tableName + ";";
	...
	static void run(PortalsFragment fragment, PortalInfo portalInfo) {
		new AsyncUpdatePortal(fragment, portalInfo).execute();
	}
}
